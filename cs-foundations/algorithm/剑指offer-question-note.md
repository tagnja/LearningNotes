...
- 解法一：排序求解。排好序的数组很容易统计出每个数字出现的次数。但需要排序。T(n) = O(nlogn), S(n) = O(1)。
- 解法二：快排划分+二分查找丢弃方法。将问题转化为求第 n/2 大的数（中位数），因为第 n/2 大的数一定是超过一半的那个数字。需要交换数组中元素的顺序。T(n) = O(n), S(n) = O(1)
- 解法三：根据数组特点求解。如果下一个数字和之前保存的数字相同，次数加一，不同则次数减一。如果次数为零，需要保存下一个数字，并把次数设为1。我们要找的数字就是最后一次把次数设为 1 时对应的数字。T(n) = O(n), S(n) = O(1)。

题目二：求数组种第 k 大的元素。

- 解法一：使用快速排序。将数组排序后求解。T(n) = O(nlogn)，S(n) = T(1)。
- 解法二：使用优先队列维护前 k 大的元素，堆中的时间复杂度为 O(k)，所以整体来说，T(n) = O(nlogk)。
- 解法三：使用快排划分+二分查找丢弃。快排划分时间复杂度为 O(nlogn)，但二分查找，每次丢弃另一部分。整体来说，T(n) = O(n)。

#### 题40：最小的 K 个数

输入n个整数，找出其中最小的k个数。例如，输入 4、5、1、6、2、7、3、8 这8个数字，最小的4个数字时 1、2、3、4。

- 解法一：排序后取前面的 k 个数字。T(n) = O(nlogn), S(n) = O(1)
- 解法二：快排划分+二分查找丢弃。使得数组左边的数字刚好是最小的 k 个数字。T(n) = O(n)
- 解法三：用一个数据容器来存储最小的k个数字。如果容器小于k个数字，读入的数字直接放入容器中。如果容器已满，则找到k个数字中的最大值，将待插入的数字和最大只进行比较，如果待插入的数字更小，则删除最大值，插入新的数字。容器可以使用二叉树最大堆来实现，复杂度为 O(logk)。总体的复杂度，T(n) = O(nlogk)。虽然这种方法比解法二的快排划分慢一点，但它没有修改数组，并适合海量数据的处理。
...
- 解法一：使用前序遍历方式来序列化二叉树，遇到 nullptr 指针序列化为特殊字符（如'$'），节点之间的数值用一个特殊字符隔开（如','）。反序列化，从数据流中每读到一个字符，判断是否是数字，如果是数字就对节点进行赋值，以及递归调用左右子树；如果不是字符则什么也不做。

#### 题41：数据流中的中位数

从一个数据流中得到中位数？如果读出的数字个数为奇数则取排序后中间的一个数字，如果是偶数则取排序后中间两个数的平均值。

- 解法一：未排序数组。复杂度：插入O(1)，得到中位数O(n)。
- 解法二：排序数组。复杂度：插入O(n)，得到中位数O(1)。
- 解法三：排序链表。复杂度：插入O(n)，得到中位数O(1)。为了得到中位数，可以定义两个指针指向链表中间的节点（如果是数据长度是奇数则指向同一个节点）
- 解法四：二叉搜索树。复杂度：插入平均 O(logn)最差O(n)，得到中位数平均O(logn)最差O(n)。为了得到中位数，可以在节点结构中添加表示子树的节点数的字段。
- 解法五：AVL 树。复杂度：插入O(logn)，得到中位数O(1)。大部分编程语言函数库没有实现这个数据结构。
- 解法六：最大堆和最小堆。复杂度：插入O(logn)，得到中位数O(1)。根据堆元素的个数得到中位数，若两个堆的元素个数一样多，取两个最值的平均值；若一个堆的元素个数大于另一个堆，取个数多的那个堆的最值。
...
输入一个整数 n，求 1~n 之间的 n 个整数中的十进制表示中 1 出现的次数。例如，输入12，包含1 的数字有 1、10、11、12，出现 1 的次数为 5 次。

- 解法一：累加1~n中每个整数1出现的次数。我们可以通过对10求余判断整数的个位数是不是1，如果整数大于10，则除以10之后再判断个位数字是不是1。
- 解法二：根据数字规律求解。。。。
...
题目一：输入一个字符串，打印出该字符串中字符的所有排列。例如，输入字符串abc，则打印abc、acb、bac、bca、cab和cba。

- 解法一：将一个字符串看成两部分，第一部分是它的第一个字符串，第二部分是后面的所有字符。第一步求所有可能出现在第一个位置的字符，即把第一个字符和后面所有的字符交换。第二步固定第一个字符，求后面所有字符的排列。这时候我们仍可以把后面的所有字符分成两个部分。我们可以通过递归的方式来求解。

题目二：求字符串的所有组合，输入abc，输出字符串的组合 a、b、c、ab、ab、bc、abc。

- 解法一：求 n 个字符中长度为 m 的组合时，把 n 个字符分成两部分，第一部分为第一个字符和第二部分为其余的所有字符。如果组合中包含第一个字符，则下一步在剩余的 n-1 字符中选取 m-1 个字符；如果组合中不包含第一个字符，则下一步在剩余的 n-1 个字符中选取 m 个字符。我们把问题规模 n 变成了多个规模为 n-1 的子问题，然后通过递归求解。

题目三：输入一个含有 8 个数字的数组，判断有没有可能把这 8 个数字分别放到正方体的 8 个顶点上，使得正方体上三组相对的面上的 4 个顶点的和都相等。

- 解法一：先得到 8 个顶点 a1, a2, a3, a4, a5, a6, a7, a8 的所有排列，然后判断有没有某一个排列符合条件，即 a1+a2+a3+a4 = a5+a6+a7+a8，a1+a3+a5+a7 = a2+a4+a6+a8，且 a1+a2+a5+a6 = a3+a4+a7+a8。

题目四：在 8*8 的国际象棋上摆放 8 个皇后，使其不能相互攻击，即任意两个皇后不得处于同一行、同一列或者同一条对角线上。求总共有多少种符合条件的摆法。

- 解法一：定义一个数组 ColumnIndex[8]，数组种的第 i 个数字表示位于第 i 行的皇后的列号。用不同的数字初始化数组，任意两个皇后肯定不在同行，同列。只需要判断每一个排列对应的 8 个皇后是不是在同一对角线上，即不能存在 i-j = ColumnIndex[i] - ColumnIndex[j]（两点斜率为1）。
...
- 解法一：举例分析数组的规律。得到如果当前数字加上之前的累加和后，总数比数字本身还小，则抛弃之前的累加。用一个变量记录连续子数组最大和。
- 解法二：动态规划。与解法一原理一样代码也一样。
